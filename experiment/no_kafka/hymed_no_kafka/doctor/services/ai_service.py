...
from hymed_no_kafka.config import DIFY_SERVER, DIFY_CT_WORKFLOW_KEY, DIFY_USER
from hymed_no_kafka.doctor.services.cache_service import new_cache_service
from hymed_no_kafka.services.event_bus_service import get_event_bus
from hymed_no_kafka.utils.dify_client import DifyClient
...
class AiService:
    ...
    async def _ai_ct_analyze_invoke(self, image_file_path: str, doctor_note: str):
        ...
        workflow_result = await self.ct_workflow.send_message(
            query=prompt,
            inputs={"doctor_note": doctor_note},
            files=[
                {
                    "type": "image",
    ...
    async def _ai_ct_analyze_task(self, image_file_path: str, context: AiCtAnalyzeContext) -> str:
        ...
        await event_bus.send(
            context.trace_id,
            {
                ...
                "target_role": "PATIENT",
                "payload": {"opd_id": context.opd_id, "doctor_note": context.doctor_note},
            },
        )
        ...
        try:
            ...
            await event_bus.send(
                context.trace_id,
                {
                    ...
                    "target_id": context.target_id,
                    "target_role": "PATIENT",
                    "payload": {"opd_id": context.opd_id, "result": result},
                },
            )
            ...
            await event_bus.send(
                context.trace_id,
                {
                    ...
                    "target_id": context.target_id,
                    "target_role": "PATIENT",
                    "payload": {"opd_id": context.opd_id, "result": None},
                },
            )
            result = "<FAILED>"
