...
class DifyClient:
    ...
    async def run_workflow(
        ...
        *,
        ...
        """
        执行 Workflow（workflows/run）。
        """
        if response_mode == "streaming":
            raise ValueError("run_workflow does not support streaming mode.")
        payload = {
            "inputs": inputs,
            "user": user,
            "response_mode": response_mode,
        }
        return await self._request("POST", "/workflows/run", json=payload)
